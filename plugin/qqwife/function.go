...
func init() {
	...
	// NTR技能
	engine.OnRegex(`(^当(\[CQ:at,qq=(\d+)\]\s?|(\d+))的小三)|(^(ntr|牛头人|NTR)(\[CQ:at,qq=(\d+)\]\s?|(\d+)))`, zero.OnlyGroup, getdb, checkMistress).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			gid := ctx.Event.GroupID
			uid := ctx.Event.UserID
			fid := ctx.State["regex_matched"].([]string)
			fiancee, e := strconv.ParseInt(fid[3]+fid[4], 10, 64)
			if e != nil {
	...
	// 做媒技能
	engine.OnRegex(`^做媒\s?\[CQ:at,qq=(\d+)\]\s?\[CQ:at,qq=(\d+)\]`, zero.OnlyGroup, getdb, checkMatchmaker).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			gid := ctx.Event.GroupID
			uid := ctx.Event.UserID
...
func checkSingleDog(ctx *zero.Ctx) bool {
	...
	// 判断CD
	ok, err := 民政局.判断CD(gid, uid, "嫁娶", dbinfo.CDtime)
	switch {
	case err != nil:
		ctx.SendChain(message.Text("[ERROR]:", err))
		return false
