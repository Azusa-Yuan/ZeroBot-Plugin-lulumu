...
func init() {
	// sus 用于后续存放解析的整数
	sus := make([]int64, 0, 16)
	// 解析命令行参数  flag包，用于解析命令行参数
	d := flag.Bool("d", false, "Enable debug level log and higher.")
	w := flag.Bool("w", false, "Enable warning level log and higher.")
	f := flag.Bool("f", false, "Enable log output to file")
	...
	if *h {
		fmt.Println("Usage:")
		flag.PrintDefaults() // 用于打印命令行的默认值和说明
		os.Exit(0)
	}
	...
	if os.IsNotExist(err) {
		errDir := os.MkdirAll(logFolder, 0755)
		if errDir != nil {
	...
	if *f {
		LogFile, err := os.OpenFile("./log/critical.log", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
		if err != nil {
	...
	// flag.Args() 返回命令行参数（不包括命令本身和已解析的标志）。range 遍历这些参数，每次迭代将一个参数赋值给 s
	...
	if *runcfg != "" {
		...
		if err != nil {
			// panic 是 Go 语言的一个内建函数，用于中止当前程序的执行。它会导致程序立即停止，并且会输出调用栈的详细信息，包括错误信息。
			panic(err)
		}
...
func main() {
	...
	// 将config加载zero process.GlobalInitMutex.Unlock的作用和意义不太明白 应该是保证所有的包都init完成？
