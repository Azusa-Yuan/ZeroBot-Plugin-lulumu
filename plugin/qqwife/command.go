...
var (
	...
	getdb = fcext.DoOnceOnSuccess(func(ctx *zero.Ctx) bool {
		...
		if err == nil {
			...
			// 创建猪头表
			err = 民政局.db.Create("piginfo", &pigInfo{})
			if err != nil {
				ctx.SendChain(message.Text("[ERROR]:", err))
				return false
			}
...
func init() {
	engine.OnFullMatchGroup([]string{"娶群友", "今日老婆"}, zero.OnlyGroup, getdb).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			...
			temp = temp[math.Max(0, len(temp)/3):]
	...
	engine.OnFullMatch("今日老公", zero.OnlyGroup).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			if rand.Float64() < 0.4 {
				uid := ctx.Event.UserID
	...
	engine.OnFullMatch("今日猪头", zero.OnlyGroup, getdb).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			uid := ctx.Event.UserID
			gid := ctx.Event.GroupID
			botId := ctx.Event.SelfID
			pigsinfon, _ := 民政局.searchPigs(gid)
			pigs := []int64{}
			if pigsinfon == (pigInfo{}) {
				// 获取群员列表
				temp := ctx.GetThisGroupMemberListNoCache().Array()
				sort.SliceStable(temp, func(i, j int) bool {
					return temp[i].Get("last_sent_time").Int() < temp[j].Get("last_sent_time").Int()
				...
				// logrus.Infoln(pigs)
				err := 民政局.savePigs(gid, pigs)
				if err != nil {
					ctx.SendChain(message.Text("[ERROR]:", err))
					return
				}
			} else {
				pigsString := pigsinfon.PigList
				err := json.Unmarshal([]byte(pigsString), &pigs)
				if err != nil {
					ctx.SendChain(message.Text("[ERROR]:", err))
					return
				}
			...
			ctx.SendChain(
				...
				message.Image("http://q4.qlogo.cn/g?b=qq&nk="+strconv.FormatInt(pigs[pigNum], 10)+"&s=640").Add("cache", 0),
				message.Text(
					"\n",
...
// 保存今日猪头的信息
func (sql *婚姻登记) savePigs(gid int64, pigs []int64) error {
	sql.Lock()
	defer sql.Unlock()

	pigsJSON, _ := json.Marshal(pigs)
	// logrus.Infoln(pigsJSON)
	pigsinfon := pigInfo{
		GID:        gid,
		PigList:    string(pigsJSON),
		Updatetime: time.Now().Format("2006/01/02"),
	}
	// logrus.Infoln(pigsinfon)
	return sql.db.Insert("piginfo", &pigsinfon)
}

// 返回今日猪头的信息
func (sql *婚姻登记) searchPigs(gid int64) (pigsinfon pigInfo, err error) {
	sql.Lock()
	defer sql.Unlock()
	err = sql.db.Find("piginfo", &pigsinfon, "where gid is "+strconv.FormatInt(gid, 10))
	if err != nil {
		return
	}
	if time.Now().Format("2006/01/02") != pigsinfon.Updatetime {
		// 如果跨天了就删除
		err = sql.db.Del("piginfo", "where gid is "+strconv.FormatInt(gid, 10))
		pigsinfon = pigInfo{}
	}
	return
}
...
func (sql *婚姻登记) 开门时间(gid int64) (dbinfo updateinfo, err error) {
	dbinfo, err = sql.查看设置(gid)
	if err != nil {
		return
	}
